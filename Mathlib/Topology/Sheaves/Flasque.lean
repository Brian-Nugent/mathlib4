...
TODO(Brian-Nugent): Show that injective sheaves are flasque.
...
open TopCat TopologicalSpace Opposite CategoryTheory AlgebraicGeometry TopCat.Presheaf

open TopCat.Sheaf.AddCommGrpCat
...
/- Given a morphism of sheaves g: F ⟶ G and a section of G(U), Under g s is comprised of an open
  V and a section of F(V) that maps to s |_ V via g -/
...
-- We given t₁ and t₂ in Under g s, we say t₁ ≤ t₂ if t₂ restricts to t₁
...
-- Here we prove that the relation R satisfies the requirements for applying Zorn's lemma
theorem Under.R.trans {a b c : Under g s} (h1 : (R g s) a b) (h2 : (R g s) b c) : (R g s) a c := by
  refine R.mk (le_trans h1.le h2.le) ?_
  rw[← h1.restricts, ← h2.restricts]
  exact Eq.symm (restrict_restrict h1.le h2.le c.sec)

theorem Under.R.chains_bounded (c : Set (Under g s)) (h : IsChain (R g s) c) :
  ...
  use ⟨iSup f, le, t, isGluing_app_of_forall_eq ht _ (fun i => i.val.app_s)⟩
  intro a ha
  constructor
  exact ht ⟨ a, ha ⟩
...
theorem epi_of_shortExact_Γ_map {S : ShortComplex (Sheaf AddCommGrpCat X)} (hS : S.ShortExact)
    (hX₁ : IsFlasque S.X₁) : Epi ((Γ U).map S.g) := by
  refine (AddCommGrpCat.epi_iff_surjective ((Γ U).map S.g)).mpr ?_
  intro s
  ...
  have : U ≤ t.V := by
    ...
    have : TopCat.Presheaf.IsLocallySurjective S.g.val := by
      ...
      exact hS.epi_g
    obtain ⟨W, i, ⟨t₁, ht₁⟩, hW⟩ :=
      (TopCat.Presheaf.isLocallySurjective_iff S.g.val).mp this U s x hx
    have := leOfHom i
    ...
    have : (S.g.val.app (op (t.V ⊓ W))) t₂ = 0 := by
      ...
      conv => rhs ; arg 1 ; simp[sheafToPresheaf_obj] ; change s |_ W
      simp only [restrict_restrict]
    obtain ⟨t₃, ht₃⟩ : ∃ t₃ : (S.X₁.val.obj (op (t.V ⊓ W))), (S.f.val.app (op (t.V ⊓ W))) t₃ = t₂ :=
      shortExact_app_zero t₂ this hS
    ...
    have app : S.g.val.app (op (iSup f)) t₅ = s |_ (iSup f) := by
      refine isGluing_app_of_forall_eq ht₅ (by rw[Fin.forall_fin_two] ; exact ⟨t.le, leOfHom i⟩) ?_
    ...
    exact (ht t₆ htt₆).le (by aesop_cat)
  use t.sec |_ U
  have : (S.g.val.app (op t.V)) t.sec |_ U = s := by
    rw[t.app_s, restrict_restrict, restrictOpen, restrict] ; aesop_cat
  conv => rhs ; rw[← this]
  rw[Γ.map_app]
  apply map_restrict

/- Given a short exact sequence of sheaves, 0 ⟶ 𝓕 ⟶ 𝓖 ⟶ 𝓗 ⟶ 0, if 𝓕 and 𝓖 are flasque, then
  𝓗 is flasque. -/
theorem X₃_shortExact_isFlasque_X₁_isFlasque_X₂ {S : ShortComplex (Sheaf AddCommGrpCat X)}
  ...
  intro U V i
  have : Epi ((Γ V).map S.g ≫ S.X₃.val.map i.op) := by
    rw[Γ.map_app, ← S.g.val.naturality i.op]
    exact CategoryTheory.epi_comp' (hX₂ i) (epi_of_shortExact_Γ_map hS hX₁)
  exact CategoryTheory.epi_of_epi ((Γ V).map S.g) (S.X₃.val.map i.op)
