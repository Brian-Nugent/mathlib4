...
theorem LinearMap.det_zero'' {R M : Type*} [CommRing R] [AddCommGroup M] [Module R M]
    [Module.IsFree R M] [Module.Finite R M] [Nontrivial M] : LinearMap.det (0 : M →ₗ[R] M) = 0 := by
  letI : Nonempty (Module.IsFree.ChooseBasisIndex R M) := (Module.IsFree.chooseBasis R M).index_nonempty
  nontriviality R
  exact LinearMap.det_zero' (Module.IsFree.chooseBasis R M)
